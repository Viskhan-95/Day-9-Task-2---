...
function getAllNames(array) {
    const nameUser = array.forEach(item => {
...
function getUserById(array, id) {
    const dataUser = array.find(item => {
        return item.id === id;
    });
...
function deleteUser(array, id) {
    const newUsers = array.filter(item => {
...
function getUsersGeo(array, id) {
    const userGeo = array.find(item => {
        return item.id === id;
    });
    return userGeo.address.geo;
}
...
function getUsersCompany(array, id) {
    const userCompany = array.find(item =>{
        return item.id === id;
    });
...
function changePhone(array, id, phone) {    
    const changePhoneUser = array.map((item) => {
        if (item.id === id) {
            item.phone = phone;
            return item;
        }
...
console.log(changePhone(users, 2, '89639804488'));
